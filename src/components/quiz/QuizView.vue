    ...
    <div class="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl shadow-xl p-6 mb-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-blue-100 text-base">{{ topic.description }}</p>
        </div>
    ...
    <div v-else>
      <div v-if="!quizComplete" class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div class="mb-6">
            ...
            <div v-if="currentQuestion.is_multiple" class="mt-3 flex items-center text-blue-600 font-medium bg-blue-50 px-3 py-2 rounded-lg inline-flex">
              <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="space-y-3 mb-6">
            ...
            >
              <div :class="[
                ...
                <div v-if="currentQuestion.is_multiple" class="mr-3 flex-shrink-0">
                  <div :class="[
                ...
                <!-- Option text -->
                <span class="flex-1 text-base text-gray-800 font-medium">{{ option }}</span>
              </div>
            </div>
          </div>

          <!-- Enhanced Feedback Section -->
          <div v-if="answerSubmitted && settings.showFeedback" class="mb-6">
            <div :class="[
              ...
              <div class="flex items-center mb-2">
                <div :class="[
                  ...
                  <svg v-if="isCorrect" class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="settings.showExplanations && currentQuestion.explanation" class="mt-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4">
              <div class="flex items-center mb-2">
                ...
                <h4 class="text-base font-semibold text-blue-800">Explanation</h4>
              </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t flex justify-between items-center">
          <button
            ...
            class="flex items-center px-4 py-2 text-gray-700 bg-white border-2 border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium"
          >
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex space-x-2">
            <button
              ...
              class="flex items-center px-6 py-2 text-white bg-gradient-to-r from-blue-600 to-purple-600 border-2 border-transparent rounded-xl hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-semibold shadow-lg"
            >
            ...
            <button
              v-else
              @click="nextQuestion"
              class="flex items-center px-6 py-2 text-white bg-gradient-to-r from-green-600 to-emerald-600 border-2 border-transparent rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 font-semibold shadow-lg"
            >
      ...
      <div v-else class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-6 text-center">
          ...
          <p class="text-green-100 text-base">Great job on completing the {{ topic.title }} quiz</p>
        </div>
        ...
        <div class="p-6">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="text-center">
              <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-4 border border-blue-200">
                <div class="text-2xl font-bold text-blue-600 mb-1">{{ Math.round(finalScore) }}%</div>
            ...
            <div class="text-center">
              <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-4 border border-green-200">
                <div class="text-2xl font-bold text-green-600 mb-1">{{ correctAnswers.length }}</div>
            ...
            <div class="text-center">
              <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-4 border border-gray-200">
                <div class="text-2xl font-bold text-gray-600 mb-1">{{ totalQuestions }}</div>
            ...
            <div class="text-center">
              <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-4 border border-purple-200">
                <div class="text-2xl font-bold text-purple-600 mb-1">{{ Math.floor(timeTaken / 60) }}:{{ String(timeTaken % 60).padStart(2, '0') }}</div>
          ...
          <div class="text-center mb-6">
            <div :class="[
              'inline-block px-6 py-3 rounded-2xl font-semibold text-base',
        ...
        <!-- Action buttons -->
        <div class="bg-gray-50 px-6 py-4 border-t flex flex-col sm:flex-row gap-3 justify-center">
          <button
            @click="goBackToTopic"
            class="flex items-center justify-center px-6 py-2 text-white bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 font-semibold shadow-lg"
          >
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="flex items-center justify-center px-6 py-2 text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 font-semibold shadow-lg"
          >
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3-3 3m0 0l-3-3m3 3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            Next Round
          </button>
          ...
          <button
            @click="restartQuiz"
            class="flex items-center justify-center px-6 py-2 text-gray-700 bg-white border-2 border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 font-semibold"
          >
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const emit = defineEmits(['complete', 'roundComplete'])

// Enhanced State with round tracking
const isLoading = ref(false)
const quizStarted = ref(false)
const quizComplete = ref(false)
const currentQuestionIndex = ref(props.resumeFromIndex || 0)
...
const roundNumber = ref(props.currentRound || 1)
...
async function startQuiz() {
  ...
  try {
    // Use questions from props
    questions.value = props.questions
    
    // Resume from saved session if applicable
    const session = StorageService.getQuizSession()
    if (session && session.answers && session.currentQuestionIndex !== undefined) {
      ...
      if (currentAnswer) {
        selectedAnswer.value = currentAnswer.selected || []
        answerSubmitted.value = !!currentAnswer
      }
    }
    
    // Update session with current state
    StorageService.updateQuizSession({
...
function finishQuiz() {
  try {
    ...
    // Create round record
    const roundData = {
      topicKey: props.topic.topic_key || props.topic.topic,
      topicTitle: props.topic.title,
      round: roundNumber.value,
      totalQuestions: totalQuestions.value,
      correctAnswers: correctAnswers.value.length,
      score: finalScore.value,
      timeStarted: new Date(quizStartTime.value).toISOString(),
    ...
    // Save round record
    const savedRound = StorageService.saveQuizRound(roundData)
...
function goBackToTopic() {
  try {
    ...
    const topicSlug = props.topic.slug || props.topic.topic_key || props.topic.topic
    router.push({
      name: 'topic',
      params: { topicSlug }
    })
...
function startNextRound() {
  try {
    // Navigate to quiz page for the next round
    const topicSlug = props.topic.slug || props.topic.topic_key || props.topic.topic
    const nextRound = roundNumber.value + 1
    
    router.push({
      name: 'quiz',
      params: { topicKey: props.topic.topic_key || props.topic.topic },
      query: {
        round: nextRound,
        ...props.settings
      }
